...
const ChromaPicker = require("./picker/chroma/ChromaPicker.js");
...
module.exports = class App extends Backbone.Model {
  ...
  setupSocket() {
    ...
    this.socket.on("reconnect-failed", () =>
      this.log("Socket gave up reconnecting")
    );
    this.socket.on("control:change", (ctrldata) => {
      ...
      if (control == null) {
        this.log(
          "warn",
          `Received update about unknown control ${ctrldata.id}`
        );
      } else {
    ...
    this.socket.on("capability:change", (capdata) => {
      ...
      if (capability == null) {
        this.log(
          "warn",
          `Received update about unknown capability ${capdata.id}`
        );
      } else {
  ...
  pickChroma(callback, current = undefined) {
    const chromaPicker = new ChromaPicker(
      { value: current },
      { callback: callback }
    );
    const view = new PagesView({ model: chromaPicker, fullSize: true });
    this.view.showModalView("Choose Color", view, { width: "450px" });
  }
  ...
  displayType() {
    ...
    } else if (this.stationConfig.has("displayType")) {
      return this.stationConfig.get("displayType");
  ...
  watchdog() {
    ...
    if (this.prevWatchdogTime != null) {
      if (
        now - this.prevWatchdogTime > 5000 &&
  ...
  isDark() {
    ...
    return (
      hour < this.stationConfig.get("darkEndHour") ||

...
module.exports = class MediaCapability extends Capability {
  ...
  currentSource() {
    ...
    let sourceId = 'single';
    if(!this.currentZoneIsSingleSource()) {
  ...
  currentZoneIsSingleSource() {
    return this.currentZoneConfig()['single-source'];
  }

  currentZoneHasVolumeControls() {
    return _.any(this.currentZoneConfig().connections,
                 (conn) => _.includes(conn.aspects, 'volume'));
  }

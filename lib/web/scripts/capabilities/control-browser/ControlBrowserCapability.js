...
module.exports = class ControlBrowserCapability extends Capability {
  ...
  initialize() {
    ...
    this.updateCards();
    this.listenTo(this.controls(), 'change', () => this.updateCards());
  }
  ...
  updateCards() {
    if(this.controlsForPowerOffCard().length > 0) {
      window.app.cards.add({
        capability: this.id,
        type: 'powerOff',
        priority: 'medium'
      });
    } else {
      const cardToRemove = window.app.cards.findWhere({
        capability: this.id, type: 'powerOff'});
      if(cardToRemove != null) window.app.cards.remove(cardToRemove);
    }
  }
}

...
module.exports = class SliderView extends Marionette.View {
  ...
  //   disabled: false     // if true, control will appear disabled
  //   gradient: undefined // if provided, acts like a color picker
  // };
  // Detent options:
  //   key:      corresponds to current value
  //   position: number min to max
  //   label:    string

  className() {
    const classes = ['slider'];
    if(this.getOption('disabled')) classes.push('disabled');
    if(this.getOption('gradient')) classes.push('color');
    return classes.join(' ');
  }
  ...
  onRender() {
    ...
    if(this.getOption('gradient')) this.setGradient(this.getOption('gradient'));
  }
  ...
  onMouseUp(event) {
    if(this.dragging) {
      ...
      if(newVal != this.value) {
        this.indicateValue(newVal, false);
        this.trigger('value:change', newVal);
      }
    }
  }
  ...
  offsetToPosition(offset) {
    const min = this.getOption('min') || 0;
    const max = this.getOption('max') || 100;
    return min + Math.max(0, Math.min(max - min, (offset / this.trackWidth())
                                      * (max - min)));
  }
  ...
  positionToOffset(position) {
    const min = this.getOption('min') || 0;
    const max = this.getOption('max') || 100;
    return ((position - min) / (max - min)) * this.trackWidth();
  }
  ...
  setGradient(gradient) {
    this.ui.track.css('background', `linear-gradient(to right, ${gradient})`);
  }
}
